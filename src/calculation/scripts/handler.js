...
module.exports = {
    findPath: function(origin,destination) {
        ...
        const startPoint = poi.find((item) => { return item.name == origin.toUpperCase()});
        if(startPoint == null){
            throw new Error('Origin not found');
        }
        const finishPoint = poi.find((item) => { return item.name == destination.toUpperCase()});
        if(finishPoint == null){
        ...
        finishPoint.distances.map( (point) => {
            const currentPoint = points.find((item) => { return item.name == point.pointName});
            currentPoint.distances.push({"pointName": finishPoint.name, "pointDistance": point.pointDistance });
        });
        ...
        result.path.map((point) => {
            if(point != 'start' && point != 'finish'){
                const currentPoint = points.find((item)=> {return item.name == point});
                parsedResult.push({name: currentPoint.name, x: currentPoint.x, y: currentPoint.y})
            }
        });
        parsedResult.push({name: destination, x: finishPoint.x, y: finishPoint.y});
        return parsedResult;
    }
}